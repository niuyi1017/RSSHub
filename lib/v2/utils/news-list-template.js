 ...
 * @param {number}   [config.limit]                 - 限制最多返回的条目数，不设置则返回全部
 * @param {boolean}  [config.admission=false]       - 研招保护：有效链接去重、最多 20 条、详情并发 3、正文体积控制；仅显式启用的路由生效
 ...
 *
 * --- HTTP（got）---
 * @param {Object}   [config.gotOptions={}]         - 传给 got 的额外选项（列表页与详情页均生效），如 { dnsLookupIpVersion: 'ipv4' } 规避部分站点 IPv6 超时
 *
 * ======================== 使用示例 ========================
 *
 ...
 */
async function fetchHTML(url, method, encoding, gotOptions = {}) {
    if (method === 'puppy') {
    ...
    const baseGot = { https: { rejectUnauthorized: false }, ...gotOptions };
    if (encoding) {
        const iconv = require('iconv-lite');
        const response = await got(url, { ...baseGot, responseType: 'buffer' });
        return iconv.decode(response.data, encoding);
    }
    // 默认 got（忽略 SSL 证书错误，部分高校站点证书过期或自签名）
    const response = await got(url, baseGot);
...
async function fetchDetailContent(itemUrl, config) {
    const { detailFetchMethod = 'got', detailContentSelector, detailExtraSelectors = [], detailParser, encoding, gotOptions = {} } = config;

    const html = await fetchHTML(itemUrl, detailFetchMethod, encoding, gotOptions);
...
function createRoute(config) {
    ...
    return async (ctx) => {
        ...
        // 1. 获取列表页 HTML
        const html = await fetchHTML(pageUrl, fetchMethod, encoding, gotOptions);
        const $ = cheerio.load(html);
        ...
        const rawItems = limit ? Array.from(list).slice(0, limit) : Array.from(list);
        ...
        const admissionFeed = admission && require('./admission-feed');
        if (admission) {
            parsedItems = admissionFeed.prepareItems(parsedItems, ctx);
            if (!parsedItems.length) {
                throw new Error(`No admission items found: ${pageUrl}`);
            }
        }
            ...
            const mapper = (parsed) =>
                ctx.cache.tryGet(parsed.link, async () => {
                    ...
                    return {
                        ...
                        description: admission ? admissionFeed.compactDescription(description, parsed.link) : description,
                    };
                });
            items = admission ? await admissionFeed.mapItems(parsedItems, mapper) : await Promise.all(parsedItems.map(mapper));
        } else {
        ...
        if (admission) {
            items = items.map((item, index) => ({
                ...item,
                title: parsedItems[index].title.trim(),
                link: parsedItems[index].link,
                pubDate: parsedItems[index].date ? timezone(parseDate(parsedItems[index].date), tz) : undefined,
                description: admissionFeed.compactDescription(item.description || parsedItems[index].title, parsedItems[index].link),
            }));
        }
