...
function defaultListItemParser($item, $, pageUrl, listParser) {
  const { dateSelector = 'span', linkSelector = 'a', titleAttr = 'title' } = listParser || {};

  const itemDate = $item.find(dateSelector).text();
...
async function fetchDetailContent(itemUrl, config) {
  const { detailFetchMethod = 'got', detailContentSelector, detailExtraSelectors = [], detailParser } = config;

  const html = await fetchHTML(itemUrl, detailFetchMethod);
...
function createRoute(config) {
  ...
  return async (ctx) => {
    ...
    // 1. 获取列表页 HTML
    const html = await fetchHTML(pageUrl, fetchMethod);
    const $ = cheerio.load(html);
