...
module.exports = async (ctx) => {
    ...
    if (response === null) {
        ...
        const items = await mapItems(list, (item) => {
            switch (item.type) {
                case 'tju-yzb':
                case 'in-site':
                    return ctx.cache.tryGet(item.link, async () => {
                        ...
                        try {
                            ...
                            item.description = compactDescription(content('.TRS_Editor').html(), item.link);
                        } catch (e) {
                            // ignore error handler
                        }
                        return item;
        ...
        ctx.state.data = {
            title: '天津大学研究生招生网 - ' + subtitle,
            link: yzb_base_url + path,
            item: items.map((item) => ({ ...item, description: compactDescription(item.description || item.title, item.link) })),
        };
    }
};
