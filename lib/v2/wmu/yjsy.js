...
module.exports = async (ctx) => {
    ...
    const typeName = $('.Convert_c_right_lm').text() || '研究生院';
    const list = $('.page_list2 ul.page_list2_c');
    const items = await Promise.all(
        Array.from(list).map((item) => {
            item = $(item);
            const itemDateDD = item.find('.dd').text();
            const itemDateYYYYMM = item.find('.mm').text();
